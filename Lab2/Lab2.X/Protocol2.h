...
// Define maximum payload length and packet buffer size
...
// Define packet buffer structure
typedef struct {
    int head;
    int tail;
    RxPacket packItems[PACKET_BUFFER_SIZE];
} PacketBuffer;
/*******************************************************************************
 * PUBLIC FUNCTIONS                                                           *
 ******************************************************************************/
...
//unsigned int Convert_int(unsigned char *c);
//void Convert_char(unsigned int val, unsigned char *c);
 ...
 * @author instructor W2022 */
int Protocol_SendPacket(unsigned char len, unsigned char ID, const unsigned char *Payload);
...
static unsigned int ConvertEndian(unsigned int num);
/*******************************************************************************
 * PRIVATE FUNCTIONS
